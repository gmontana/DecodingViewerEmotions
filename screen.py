...
def main():
    ...
    screen_p1 =  screen_emotion_ids = [ "p" ] # [ 0, 1, 2, 3, 4 , 5,  7,   8]#[1, 3, 4, 5] # # # [7] #
    screen_p2 =  screen_jump = [  0.1,    0.5,  1,2,3    ]
    #screen_p2 =  screen_porog = [0.2]
    screen_p3 =  screen_dropout = [   0.5]  #inet->0.8, timm 0.5 screen_dropout # screen_model_segment_config  # screen_dataset_n_mels # screen_model_net_param_shift_temporal_modality_f_div #screen_arch #screen_dropout # screen_video_transform_param_RandomHorizontalFlip #screen_arch #screen_consensus_type # screen_net_optim_param_epochs_decay_start #
    screen_p4 =  screen_lr = [ 0.1 ] #inet-> 0.01, timm->0.1
    screen_p5 =  screen_last_pool = [  1]
    screen_p6 =  screen_arch = ["resnet50_timm" ] #, "resnet50_timm","densenet201","resnet50_timm" "densenet201"  #"densenet201"
    screen_p7 =  screen_segment_config = [ [16,0,1] ] # , [16,0,0] [8,0,1], [8,0,0], [4,0,1], [4,0,0]
         ...
         for i6, p6 in enumerate(screen_p6):
          for i7, p7 in enumerate(screen_p7):
             ...
             for rid in range(number_of_runs):
                ...
                screen_id = f'{rid}'
                pstr_list = [p2str(p1), p2str(p2), p2str(p3), p2str(p4), p2str(p5), p2str(p6) , p2str(p7)]
                ...
                if args_in.cuda_ids: cmd += f" --cuda_ids {args_in.cuda_ids}"


                args["run_id"] = screen_id
                args = define_rootfolders(args)
                print("result.json:", f'{args["output_folder"]}/result.json')
                ...
                print("file_results_screen", file_results_screen)
