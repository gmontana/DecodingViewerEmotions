            ...
            if (len(fileA) > 2):
                ...
                if os.path.isdir(path + "/" + fileA):
                    ...
                    for fileB in path_ids2:
                        if os.path.isdir(f"{path}/{fileA}/{fileB}"):
                            continue
...
def get_num_class(args):
    ...
    file_train_list = f'{param_dataset["data_dir"]}/{param_dataset["file_train_list"]}'
...
def estimate_accuracy(Y_true_max, Y_pred_max, v):
    ...
    Y_true_label, Y_pred_label = get_labels(
        Y_true_max, Y_pred_max, threthholds)
...
def remap_acc_to_emotions(acc, ValidDataSet):
    accE = {}
    for i, v in enumerate(acc):
        eid = ValidDataSet.map_label[i]
        emotion = ValidDataSet.video_db.Emotions.mapE[eid]
        accE[emotion] = v
        print(f"accE {emotion} {v}")
    return accE
